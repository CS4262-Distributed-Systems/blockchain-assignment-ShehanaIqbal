...
public final class AssetTransfer implements ContractInterface {
    ...
    /**
     * Create a duplicate of the given asset with new owner.
     *
     * @param ctx the transaction context
     * @param assetID the ID of the asset being updated
     * @param owner the owner of the asset being updated
    ...
    public Asset DuplicateAsset(final Context ctx, final String assetID, final String owner) {
        ...
        Asset existingAsset = genson.deserialize(assetJSON, Asset.class);

        // Find a new unique id for the new asset
        String newAssetId = GetUniqueAssetId(ctx);

        // Create a new asset with new unique id
        Asset duplicateAsset = CreateAsset(ctx, newAssetId, existingAsset.getColor(), existingAsset.getSize(), owner, existingAsset.getAppraisedValue());

        return duplicateAsset;
    }

    /**
     * Find a unique asset id.
     *
     * @param ctx the transaction context
     * @return the unique asset id
     */
    @Transaction(intent = Transaction.TYPE.SUBMIT)
    public String GetUniqueAssetId(final Context ctx) {
        int uniqueId = 0;

        // Finding a unique asset id by incrementing the post substring
        while (true) {
